...
export async function POST(request) {
    try {
        ...
        const passwordMatch = await isValidPassword(user[0], String(password));
        if ( !passwordMatch ) {
        ...
        const token = jwt.sign({ id: user.id, role: user.role, plan: user.plan }, process.env.COOKIE_KEY, { expiresIn: "30m" });
